...
barba.hooks.afterEnter(data => {
  ...
  scheduleInitialHashScroll(data.next.container);
...
function getInitialHash() {
  return pendingAnchorHash || window.location.hash || null;
}
...
function scheduleAnchorScroll(hash, container = document, { smooth = false } = {}) {
  ...
  const run = (immediate) => {
    ...
    if (didScroll) syncUrlHash(hash);
    return didScroll;
  };

  const didScroll = run(!smooth);

  [50, 250, 750, 1500].forEach(delay => {
    const timer = setTimeout(() => run(true), delay);
...
function initAnchorLinkHandling() {
  document.addEventListener("click", (event) => {
    ...
    scheduleAnchorScroll(normalizedUrl.hash, document, { smooth: true });
  }, true);
