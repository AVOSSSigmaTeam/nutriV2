...
const initialLocationHash = window.location.hash || null;
let pendingAnchorHash = initialLocationHash;
const homePathAliases = new Set(["/", "/index", "/index.html"]);
let anchorScrollTimeouts = [];
let anchorNavigationId = 0;
const anchorTrace = [];

window.__nutriPageTransitionAnchorTrace = anchorTrace;

if (initialLocationHash) {
  suppressNativeInitialHashScroll(initialLocationHash);
}
...
function runPageOnceAnimation(next) {
  ...
  tl.call(() => {
    resetPage(next, { scroll: false, source: "once:reset" });

    const hash = getInitialHash();
    if (hash) {
      performAnchorScroll(hash, next, { behavior: "auto", source: "once:anchor" });
    } else {
      scrollToPageTop({ source: "once:top" });
    }
  }, null, 0);
...
function runPageEnterAnimation(next) {
  ...
  const targetHash = getInitialHash();
  ...
  if (reducedMotion) {
    ...
    tl.add("pageReady");
    tl.call(() => {
      resetPage(next, { scroll: false, source: "enter:reduced:reset" });

      const hash = getInitialHash();
      if (hash) {
        performAnchorScroll(hash, next, { behavior: "auto", source: "enter:reduced:anchor" });
      } else {
        scrollToPageTop({ source: "enter:reduced:top" });
      }
    }, null, "pageReady");
  ...
  tl.call(() => {
    resetPage(next, { scroll: false, source: "enter:prepare:reset" });

    const hash = getInitialHash();
    if (hash) {
      performAnchorScroll(hash, next, { behavior: "auto", source: "enter:prepare:anchor" });
    } else {
      scrollToPageTop({ source: "enter:prepare:top" });
    }
  }, null, "startEnter-=0.05");
  ...
  tl.add("pageReady");
  tl.call(() => {
    verifyAnchorScroll(targetHash, next, "enter:pageReady");
  }, null, "pageReady");
...
barba.hooks.before(data => {
  pendingAnchorHash = getNavigationHash(data?.trigger) || null;
...
barba.hooks.afterEnter(data => {
  ...
  verifyAnchorScroll(window.location.hash, data.next.container, "afterEnter");
...
function resetPage(container, { scroll = true, source = "resetPage" } = {}) {
  gsap.set(container, {
  ...
  if (scroll) {
    if (getInitialHash()) {
      performAnchorScroll(getInitialHash(), container, { behavior: "auto", source: `${source}:anchor` });
    } else {
      scrollToPageTop({ source: `${source}:top` });
    }
  }
  ...
  anchorLog("page:reset", { source, hash: getInitialHash(), container });
...
function suppressNativeInitialHashScroll(hash) {
  if (!hash || hash === "#") return;

  const url = new URL(window.location.href);
  url.hash = "";
  history.replaceState(history.state, "", url.href);
  anchorLog("initial-hash:suppressed-native", { source: "bootstrap", hash });
}

function getLenisState() {
  if (!hasLenis || !lenis) return null;
  ...
  return {
    ...
    actualScroll: Math.round(lenis.actualScroll || 0),
    limit: Math.round(lenis.limit || 0),
...
function getAnchorSnapshot(hash, container = document) {
  const target = hash ? getHashTarget(hash, container) : null;
  const targetRect = target ? target.getBoundingClientRect() : null;

  return {
    href: window.location.href,
    hash: hash || window.location.hash || null,
    pendingAnchorHash,
    scrollY: Math.round(window.scrollY),
    targetExists: !!target,
    targetTop: targetRect ? Math.round(targetRect.top + window.scrollY) : null,
    targetRectTop: targetRect ? Math.round(targetRect.top) : null,
    bottomDistance: Math.round(document.documentElement.scrollHeight - window.innerHeight - window.scrollY),
...
function anchorLog(label, details = {}) {
  if (!ANCHOR_DEBUG) return;

  const entry = {
    id: details.id || anchorNavigationId || null,
    label,
    source: details.source || null,
    time: Math.round(performance.now()),
    ...getAnchorSnapshot(details.hash, details.container),
  };

  anchorTrace.push(entry);
  if (anchorTrace.length > 400) anchorTrace.shift();

  console.log(`[PageTransition ${version}] ${label}`, entry);
}
...
function getHashTargetInfo(hash, container = document) {
  const target = getHashTarget(hash, container);
  if (!target) return null;

  return {
    target,
    targetY: clampScrollY(target.getBoundingClientRect().top + window.scrollY),
  };
}

function cancelLenisScroll(source = "cancelLenisScroll") {
  if (!hasLenis || !lenis) return;

  lenis.animate?.stop?.();
  lenis.reset?.();
  anchorLog("lenis:cancel-scroll", { source });
}

function scrollToPageTop({ source = "scrollToPageTop" } = {}) {
  cancelLenisScroll(source);
  ...
  anchorLog("scroll:top", { source });
}

function scrollToHash(hash, container = document, { immediate = false } = {}) {
  return performAnchorScroll(hash, container, {
    behavior: immediate || reducedMotion ? "auto" : "smooth",
    source: immediate ? "scrollToHash:immediate" : "scrollToHash:smooth",
  });
}

function shouldDeferHashSync(source) {
  return source?.startsWith?.("once:") && document.readyState !== "complete";
}

function syncUrlHash(hash, { source = "syncUrlHash", defer = false } = {}) {
  if (!hash || hash === "#" || window.location.hash === hash) return;

  if (defer) {
    const restore = () => {
      requestAnimationFrame(() => {
        syncUrlHash(hash, { source: `${source}:after-load`, defer: false });
      });
    };
    ...
    anchorLog("url:hash-sync-deferred", { source, hash });
    return;
  }
  ...
  url.hash = hash;
  history.replaceState(history.state, "", url.href);
  anchorLog("url:hash-synced", { source, hash });
}

function performAnchorScroll(hash, container = document, { behavior = "auto", source = "performAnchorScroll" } = {}) {
  if (!hash || hash === "#") {
    pendingAnchorHash = null;
    anchorLog("anchor:skip-empty", { source, hash, container });
    return false;
  }

  clearAnchorScrollTimers();
  const id = ++anchorNavigationId;
  const info = getHashTargetInfo(hash, container);

  anchorLog("anchor:start", { id, source, hash, container });

  if (!info) {
    pendingAnchorHash = null;
    anchorLog("anchor:target-missing", { id, source, hash, container });
    return false;
  }

  syncUrlHash(hash, { source, defer: shouldDeferHashSync(source) });
  cancelLenisScroll(source);

  const shouldSmooth = behavior === "smooth" && !reducedMotion;

  if (hasLenis && lenis) {
    lenis.start();
    lenis.resize();

    lenis.scrollTo(info.targetY, {
      immediate: !shouldSmooth,
      duration: shouldSmooth ? 1 : undefined,
      force: true,
      lock: false,
      onStart: () => anchorLog("anchor:lenis-start", { id, source, hash, container }),
      onComplete: () => {
        anchorLog("anchor:lenis-complete", { id, source, hash, container });
        verifyAnchorScroll(hash, container, `${source}:complete`);
      },
    });
  } else if (shouldSmooth) {
    window.scrollTo({ top: info.targetY, behavior: "smooth" });
  } else {
    window.scrollTo(0, info.targetY);
  }

  pendingAnchorHash = null;
  anchorLog(shouldSmooth ? "anchor:smooth-requested" : "anchor:jumped", { id, source, hash, container });

  if (!shouldSmooth) {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => verifyAnchorScroll(hash, container, `${source}:raf-verify`));
    });
    queueAnchorSettleChecks(hash, container, source);
  }
...
function getInitialHash() {
  return pendingAnchorHash || null;
}

function clearAnchorScrollTimers() {
  anchorScrollTimeouts.forEach(timer => clearTimeout(timer));
  anchorScrollTimeouts = [];
}

function queueAnchorSettleChecks(hash, container = document, source = "anchor:settle") {
  if (!hash || hash === "#") return;

  const runCheck = (delay) => {
    const timer = setTimeout(() => settleAnchorScroll(hash, container, `${source}:settle-${delay}`), delay);
    anchorScrollTimeouts.push(timer);
  };

  if (document.readyState === "complete") {
    [80, 300, 900].forEach(runCheck);
    return;
  }

  window.addEventListener("load", () => {
    [0, 100, 350, 900].forEach(runCheck);
  }, { once: true });

  anchorLog("anchor:settle-queued-after-load", { source, hash, container });
}

function settleAnchorScroll(hash, container = document, source = "anchor:settle") {
  const snapshot = getAnchorSnapshot(hash, container);
  if (!snapshot.targetExists) {
    anchorLog("anchor:settle-target-missing", { source, hash, container });
    return false;
  }

  if (Math.abs(snapshot.targetRectTop) <= 3) {
    anchorLog("anchor:settle-noop", { source, hash, container });
    return true;
  }

  const info = getHashTargetInfo(hash, container);
  if (!info) {
    anchorLog("anchor:settle-target-info-missing", { source, hash, container });
    return false;
  }

  anchorLog("anchor:settle-correct", { source, hash, container });
  cancelLenisScroll(source);
  ...
  verifyAnchorScroll(hash, container, `${source}:after-correct`);
  return true;
}

function scheduleAnchorScroll(hash, container = document, { smooth = false } = {}) {
  return performAnchorScroll(hash, container, {
    behavior: smooth ? "smooth" : "auto",
    source: smooth ? "scheduleAnchorScroll:smooth" : "scheduleAnchorScroll:auto",
  });
}

function scheduleInitialHashScroll(container = document) {
  return scheduleAnchorScroll(getInitialHash(), container, { smooth: false });
}

function verifyAnchorScroll(hash, container = document, source = "verifyAnchorScroll") {
  if (!hash || hash === "#") return;

  const snapshot = getAnchorSnapshot(hash, container);
  const isSettled = snapshot.targetExists && Math.abs(snapshot.targetRectTop) <= 3;

  anchorLog(isSettled ? "anchor:verified" : "anchor:verify-miss", { source, hash, container });

  return isSettled;
}

function closeMobileNavForAnchor(link) {
  ...
  navMenuButton.click();
  anchorLog("mobile-nav:closed-for-anchor", { source: "same-page-anchor-click", hash: pendingAnchorHash });
}
...
function initAnchorLinkHandling() {
  document.addEventListener("click", (event) => {
    ...
    if (!normalizedUrl.hash || normalizedUrl.hash === "#") {
      ...
      pendingAnchorHash = null;
      anchorLog("anchor:empty-click-prevented", { source: "same-page-anchor-click", hash: normalizedUrl.hash });
      return;
    }
    ...
    closeMobileNavForAnchor(link);
    performAnchorScroll(normalizedUrl.hash, document, { behavior: "smooth", source: "same-page-anchor-click" });
  }, true);
